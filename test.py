...
def getZScore(value, reference):
    """for a given value and a set of reference values return zscore

    Args:
        value (float): counts for target
        reference (list): list of counts for references
...
def loadSample(testData, tChrom):
    ...
    if tChrom == "X":
        sumCount = float(
            sum([sum(testSample[x]) for x in testSample.keys()])
...
def cnvTest(
    ignoreBins, probeInfo, testSample, reference, tChrom, refCutOff, directCalls
):
    ...
    for i in range(len(relative)):
        if (
            abs(byTarget[i]) > thresHold
            and abs(byRelative[i]) > relThresh
            and i not in ignoreBins > 0
        ):
            called.append(i)
    ...
    for i, region in enumerate(mapZRegions):
        ...
        outSide = [min(1.0, x) for x in outSide]
        inSide = [min(1.0, x) for x in inSide]
        ...
        for x in range(0, len(extendedRegion) - maxRange):
            for y in range(max(x + 1, maxRange), len(extendedRegion)):
                ...
                out = numpy.mean(
                    reducedOutside + extendedRegion[:x] + extendedRegion[y:]
                )
                ...
                if (
                    abs(mid) < relThresh
                    or abs(numpy.median(extendedRegion[x:y])) < relThresh
                ):
                    diff = 0
                # Don't bother with some end points if they don't fulfill our threshold anyway
                if (
                    abs(extendedRegion[x]) < relThresh
                    or abs(extendedRegion[y - 1]) < relThresh
                    or (extendedRegion[x] > 0) != (mid > 0)
                    or (extendedRegion[y - 1] > 0) != (mid > 0)
                ):
                    diff = 0
                # Update our champion
    ...
    if miniCalls != [] and miniCalls[-1][1] >= len(noNanI):
        print(
            "This call is fishy:",
            miniCalls[-1],
            "It ends beyond the last testable probes?",
        )
    ...
    means = [
        numpy.mean(noNanR[x[0]: min(len(noNanI) - 1, x[1] + 1)]) for x in miniCalls
    ]

    # extCall = called[:]
    maxDistances = [
        noNanI[min(len(noNanI) - 1, x[1] + 1)] - noNanI[x[0] - 1] for x in miniCalls
    ]
...
def main():
    ...
    parser.add_argument(
        "testfile", type=str, help="file containing amount of hits per probe"
    )
    parser.add_argument(
        "reffile", type=str, help="file containing reference probes per probe target"
    )
    parser.add_argument("dropfile", type=str, help="file to save output")
    parser.add_argument("targetchr", type=str, help="chromosome to target bins on")
    parser.add_argument("probefile", type=str, help="bed file with probe locations")
    parser.add_argument("exonfile", type=str, help="bed file with exon information")
    parser.add_argument(
        "occfile",
        type=str,
    ...
    parser.add_argument(
        "-plot", action="store_true", help="plot the data interactively"
    )
    parser.add_argument(
        "-plotfile", action="store_true", help="plot the data to a file"
    )
    parser.add_argument(
        "-direct",
        action="store_true",
    ...
    if makePlot or makePlotFile:
        ...
        overviewPlot = cnvplot.overviewPlot(
            byTarget, refStdDev, byRelative, thresHold, relThresh
        )
        ...
        for calledIndex, region in enumerate(regional):
            ...
            print(calledIndex, region, nonOccRegion[calledIndex], end=" ")
            if (
                region[1] - region[0] - nonOccRegion[calledIndex] < 3
            ):  # or nonOccRegion[calledIndex] < 0.75:
                print("skipped")
                # continue
            print("kept")
            regionPlots.append(
        ...
        if makePlotFile:
            ...
            for i, reg in enumerate(regionPlots):
                plt.figure(
                    i + 3
                )  # Due to some undefined logic apparently we can't use figs added to the list previously
                plt.savefig(
                    fileDrop
                    + "_"
                    + str(probeInfo[reg[0]][0])
                    + "-"
                    + str(probeInfo[reg[1]][1])
                    + ".pdf"
                )
