...
def flamePlot(mapZ, noNanZ, markRegions, flatten, colorCeil):
    ...
    plt.imshow(
        markRegions, cmap=get_cmap(fColorMap), interpolation="nearest", aspect="auto"
    )
    plt.clim(-colorCeil, colorCeil)
...
def overviewPlot(byTarget, refStdDev, byRelative, thresHold, relThresh):
    ...
    plt.scatter(
        range(len(byTarget)), byTarget, marker="x", c=colorMap, cmap=plt.cm.coolwarm
    )
    ...
    plt.scatter(
        range(len(byRelative)), byRelative, marker="x", c=colorMap, cmap=plt.cm.coolwarm
    )
...
def regionPlot(
    ...
    plt.imshow(
        miniVals, cmap=get_cmap(fColorMap), interpolation="nearest", aspect="auto"
    ...
    plt.subplot(gs[1])
    plt.imshow(
        [[x[1] for x in mapZMaxes[miniCall[0] - leftHang: miniCall[1] + rightHang]]],
        cmap=get_cmap(fColorMap),
        interpolation="nearest",
        aspect="auto",
    )  # interpolation='nearest',
    plt.clim(-colorCeil + flatten, colorCeil - flatten)
    ...
    marker = (
        [0] * leftHang + [1] * (miniCall[1] - miniCall[0] + 1) + [0] * (rightHang - 1)
    )

    plt.imshow(
        [marker], cmap=get_cmap(fColorMap), interpolation="nearest", aspect="auto"
    )  # interpolation='nearest',
    plt.clim(-1, 1)
    # plt.colorbar(orientation='vertical')
    plt.tight_layout()
    ...
    for i in range(1, len(byRegion)):
        if (
            byRegion[i][0] > regStart - regHang
            and byRegion[i - 1][1] < regEnd + regHang
        ):
            plt.plot(
                [byRegion[i - 1][1], byRegion[i][0]],
                [byRelative[i - 1], byRelative[i]],
                color=surroundColor,
                linestyle="-",
                linewidth=1,
            )
    ...
    for i, val in enumerate(exonInfo):
        ...
        if val[1] > regStart - regHang and val[0] < regEnd + regHang:
            ...
            while (
                probeInfo[probeFinger][1] < val[0] and probeFinger < len(probeInfo) - 1
            ):
                probeFinger += 1

            while (
                probeInfo[probeFinger][0] < val[1] and probeFinger < len(probeInfo) - 1
            ):
                thisProbe = [
                    probeInfo[probeFinger][0],
                    probeInfo[probeFinger][1],
                    relative[probeFinger],
                ]
    ...
    for i in range(region[0] + 1, region[1] + 1):
        if (
            ...
            math.isnan(byRelative[i]) or byRelative[i] == 0.0 or i in ignoreBins
        ):
            plt.plot(
                [lastPos, byRegion[i][0]],
                [lastVal, byRelative[i]],
    ...
    maxCNV = max(
        [byRelative[x] for x in range(region[0], region[-1] + 1) if x not in ignoreBins]
    )
