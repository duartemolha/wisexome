...
def writeToPickle(
    ...
    for i, region in enumerate(regional):
        ...
        exportDict["median"] = numpy.median(
            [x for x in byRelative[region[0] : region[-1] + 1] if not math.isnan(x)]
        )
...
def writeToBed(fileDrop):
    ...
    with open(fileDrop + ".bed", "w") as toBedFile:
        ...
        for i, val in enumerate(byRegion):
            ...
            colorRGB = [str(int(x * 255.0)) for x in colorBED]
            # print bedScore
            outLine = [
                tChrom,
                val[0],
                ...
                val[0],
                ",".join(colorRGB),
            ]  # ,len(bedProbePos),','.join(bedProbeLen),','.join(bedProbePos)]
    ...
    with open(fileDrop + "_short.bed", "w") as toBedFile:
        for region in regional:
            ...
            print(
                len(byRegion),
                region[1] + 2,
                min(len(byRegion) - 1, region[1] + 2),
                byRegion[min(len(byRegion) - 1, region[1] + 2)][0],
            )
