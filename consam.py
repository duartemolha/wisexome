...
parser = argparse.ArgumentParser(
    description="Convert any stream of reads to a pickle file for WISExome, defaults are set for the SAM format",
    formatter_class=argparse.ArgumentDefaultsHelpFormatter,
)

parser.add_argument("bamfile", type=str, help="bam file for conversion")
parser.add_argument("bedfile", type=str, help="bed file for targets")
parser.add_argument("outfile", type=str, help="count file for targets")
    ...
    if chrom[3:] in targets:
        ...
        for read in samfile.fetch(chrom):
            if (
                ...
                # Pick closest probe
                if abs(midRead - (curTarget[0] + curTarget[1]) / 2) > abs(
                    midRead - (lastTarget[0] + lastTarget[1]) / 2
                ):
                    # Check if read is fully overlapping probe
                    # if lastTarget[0] >= read.pos and lastTarget[1] <= read.pos + read.isize:
                    if (
                        lastTarget[1] >= read.pos + 20
                        and lastTarget[0] <= read.pos + read.isize - 20
                    ):
                        lastCount += 1
                    ...
                    if (
                        curTarget[1] >= read.pos + 20
                        and curTarget[0] <= read.pos + read.isize - 20
                    ):
                        curCount += 1
