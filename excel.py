...
with open(fileDrop + ".xls", "w") as toXlsFile:
    ...
    for sampleI, sampleName in enumerate(sampleNames):
        ...
        for call in dataSets[sampleI]:
            ...
            for sampleJ, otherSampleName in enumerate(sampleNames):
                if sampleJ != sampleI:
                    ...
                    for otherCall in dataSets[sampleJ]:
                        if (
                            call["chromosome"] == otherCall["chromosome"]
                            and call["minEnd"] > otherCall["minStart"]
                            and call["minStart"] < otherCall["minEnd"]
                        ):
            ...
            ucscLink = (
                'HYPERLINK("https://genome-euro.ucsc.edu/cgi-bin/hgTracks?db=hg19&position=chr%s'
                + ":"
                + "%s"
                + "-"
                + '%s";"%s")'
            ...
            for gene in call["genes"]:
                print("\n\n" + gene)
                url = (
                    "http://api.europe.omim.org/api/entry/search?search=gene_symbol:"
                    + gene
                    + "+AND+prefix:*&include=geneMap&format=python&apiKey="
                    + apiKey
                )
                    ...
                    for stepOne in omimObject["omim"]["searchResponse"]["entryList"]:
                        ...
                        if "geneMap" in stepOne["entry"]:
                            if "phenotypeMapList" in stepOne["entry"]["geneMap"]:
                                ...
                                for stepTwo in stepOne["entry"]["geneMap"][
                                    "phenotypeMapList"
                                ]:
                                    worstCase = max(
                                        worstCase,
                                        stepTwo["phenotypeMap"]["phenotypeMappingKey"],
                                    )
                                    subCase = max(
                                        subCase,
                                        stepTwo["phenotypeMap"]["phenotypeMappingKey"],
                                    ...
                                    phenoType = (
                                        phenoType
                                        + "; "
                                        + stepTwo["phenotypeMap"]["phenotype"]
                                    )
                        # if gene!='':
                        # print gene
            ...
            for gene in sorted(geneList, reverse=True):
                ...
                if gene[1] != 0:
                    ...
                    omimLink = omimLink % (
                        gene[2],
                        "?: " + gene[2],
                    )  # 'chr'+str(tChrom)+':'+str(posStart)+'-'+str(posEnd))
                    ws.write(r, c, Formula(omimLink), ulstyle)
                # ws.write(r,c,gene)
                c += 1
